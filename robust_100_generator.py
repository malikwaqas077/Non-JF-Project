#!/usr/bin/env python3
"""
Robust 100 image generator with retry logic and progress tracking
"""
...
print("="*70)
print("ROBUST 100 HIGH GAS MASS IMAGE GENERATOR")
print("With retry logic and progress tracking")
print("="*70)
...
def make_vis_url(base: str) -> str:
    ...
    params = dict(
        ...
        size="0.25", sizeType="rViral", depthFac="1",
...
print(f"\nFound {len(entries)} entries in CSV")
...
for i, (filename, base_url) in enumerate(entries):
    if filename in existing_images:
        print(f"[{i+1}/{len(entries)}] {filename} (subhalo {filename.split('_')[2].split('.')[0]}) - SKIPPED (already exists)")
        success_count += 1
        continue
    
    print(f"[{i+1}/{len(entries)}] {filename} (subhalo {filename.split('_')[2].split('.')[0]})")
    
    try:
        url = make_vis_url(base_url)
    ...
    if (i + 1) % 10 == 0:
        print(f"\n  Progress: {i+1}/{len(entries)} processed, {success_count} successful, {visible_gas_count} with visible gas")

print(f"\n" + "="*70)
print("RESULTS")
print("="*70)
print(f"Successfully saved: {success_count}/{len(entries)}")
